...
import {
  VStack, HStack, Box, Flex, SimpleGrid, Text, IconButton,
...
function SessionBookingsView() {
  ...
  const fetchBookings = async () => {
    try {
      const res = await axios.get("/ymsapi/admin/session-bookings/", {
        headers: { Authorization: `Bearer ${token}` },
      });
      setBookings(res.data || []);
    } catch (err) {
      toast({ title: "Error loading bookings", status: "error", duration: 3000, isClosable: true });
    } finally {
      setLoading(false);
    }
  };
  ...
  const updatePaymentStatus = async (regId, newStatus) => {
    try {
      await axios.put(
        `/ymsapi/admin/update-payment-status/session/${regId}/`,
        { status: newStatus },
        { headers: { Authorization: `Bearer ${token}` } }
      );
      toast({
  ...
  return (
    <VStack gap="20px" width="100%" align="stretch">
      <HStack justify="space-between" align="center">
        <TextP heading="Session Bookings" fw="600" fs="24px" lh="36px" />
        <Box
          bg="#ECFDF3"
          borderRadius="20px"
          px="16px"
      ...
      {bookings.length === 0 ? (
        <Box p="40px" textAlign="center" bg="white" borderRadius="10px" border="1px solid #E2E8F0">
          <TextP heading="No session bookings yet." fw="400" fs="16px" lh="24px" />
        </Box>
      ) : (
        <VStack gap="12px" width="100%">
          ...
          <SimpleGrid
            ...
            gap="8px"
          >
            {["Name", "Email", "Phone", "Batch Time", "Payment", "Date", "Action"].map((h) => (
          ...
          {bookings.map((b) => (
            <Box key={b.id} width="100%">
              ...
              >
                ...
                <Text fontFamily="Poppins" fontSize="13px" fontWeight="500">{b.batch_time}</Text>
                <Box>
                  <Select
                    size="sm"
              ...
              >
                <VStack align="stretch" gap="8px">
                  ...
                  <Text fontFamily="Poppins" fontSize="13px" color="#666">{b.email}</Text>
                  <HStack justify="space-between">
                    <Text fontFamily="Poppins" fontSize="13px">🕐 {b.batch_time}</Text>
                    <Text fontFamily="Poppins" fontSize="12px" color="#999">
                      {b.created_at ? new Date(b.created_at).toLocaleDateString() : ""}
                    </Text>
                  </HStack>
...
export default SessionBookingsView;
